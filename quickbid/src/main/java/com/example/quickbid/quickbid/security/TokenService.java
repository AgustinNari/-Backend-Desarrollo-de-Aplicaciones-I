...
public class TokenService {
	...
	public TokenService(
			...
			@Value("${app.jwt.access-token-minutes:15}") int minutes) {
		if (secret.length() < 32) {
			throw new IllegalArgumentException("APP_JWT_SECRET must contain at least 32 characters");
		}
	...
	public String access(Long id, String email, String estado) {
		try {
			String h = b64(json.writeValueAsBytes(Map.of("alg", "HS256", "typ", "JWT")));
			long now = Instant.now().getEpochSecond();
			String p = b64(json.writeValueAsBytes(Map.of(
					"sub", id.toString(),
					"email", email,
					"estado", estado,
					"iat", now,
					"exp", now + accessMinutes * 60L)));
			return h + "." + p + "." + b64(sign(h + "." + p));
		} catch (Exception e) {
		...
		try {
			...
			if (
					...
					!MessageDigest.isEqual(
							sign(p[0] + "." + p[1]),
							Base64.getUrlDecoder().decode(p[2]))) {
				throw new IllegalArgumentException();
			}

			Map<?, ?> claims = json.readValue(Base64.getUrlDecoder().decode(p[1]), Map.class);
