...
@RestController @RequestMapping("/api/auth") public class AuthController {
 ...
 @PostMapping(value="/registro/etapa2",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) public ApiResponse<Void> etapa2(@RequestParam String email,@RequestPart(required=false) MultipartFile fotoFrenteDni,@RequestPart(required=false) MultipartFile fotoDorsoDni,@RequestPart(required=false) MultipartFile fotoFrente,@RequestPart(required=false) MultipartFile fotoDorso){auth.etapa2(email,file(fotoFrenteDni,fotoFrente),file(fotoDorsoDni,fotoDorso));return ApiResponse.success(null,"Documentacion recibida");}
 ...
 @PostMapping("/registro/etapa3") public ApiResponse<Map<String,Object>> etapa3(@RequestBody PasswordChange r){return ApiResponse.success(auth.etapa3(r.setup(),r.clave(),r.confirmacion()),"Cuenta creada");}
 ...
 @PutMapping("/cambiar-clave") public ApiResponse<Void> change(Authentication a,@RequestBody PasswordChange r){if(a==null||!(a.getPrincipal() instanceof Long))auth.changeWithToken(r.token(),r.nueva(),r.confirmacion());else auth.changeAuthenticated((Long)a.getPrincipal(),r.claveActual(),r.nueva(),r.confirmacion());return ApiResponse.success(null,"Clave actualizada");}
 ...
 public record PasswordChange(@JsonProperty("setup_token") String setupToken,String token,String clave,String claveActual,String claveNueva,String claveConfirmacion){String setup(){return setupToken!=null?setupToken:token;}String nueva(){return claveNueva!=null?claveNueva:clave;}String confirmacion(){return claveConfirmacion!=null?claveConfirmacion:clave;}}
 public record Refresh(@JsonProperty("refreshToken") @NotBlank String refreshToken){}
 private MultipartFile file(MultipartFile primary,MultipartFile secondary){MultipartFile value=primary!=null?primary:secondary;if(value==null||value.isEmpty())throw new BusinessException(HttpStatus.BAD_REQUEST,"Falta archivo requerido","MISSING_FILE");return value;}
}
