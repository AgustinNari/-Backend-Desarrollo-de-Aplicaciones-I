...
public interface DireccionEnvioRepository extends JpaRepository<DireccionEnvio, Long> {
	...
	@Query(
			value = "UPDATE app_direcciones_envio SET principal=false,updated_at=CURRENT_TIMESTAMP " +
					"WHERE cuenta_id=:cuentaId AND id<>:selectedId AND principal=true AND deleted_at IS NULL",
			nativeQuery = true)
